    ...
    "must display the page when the user is authorised and there are features for the service" in:
      ...
      summaryList.foreach:
        case FeatureSummary(feature, _) =>
          ...
          Option(html.getElementById(s"$service-$feature-increase-new-user-limit")).value.attributes().get("href") mustEqual routes.IndexController.increaseNewUserLimit(service, feature).url
